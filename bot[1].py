...
async def query_ragemp_server(ip_port):
    ...
    try:
        ...
        print(f"[Query] Versuche Server abzufragen: {host}:{port}")
        
        # Methode 1: Socket Test (prüft ob Port offen ist)
        try:
            import socket
            ...
            if result == 0:
                print(f"[Query] ✓ Server ist online (Port offen)")
                # Server ist erreichbar, aber wir können keine Spielerzahl abrufen
                # Das ist normal bei RageMP Servern ohne öffentliche API
                return {
                    'online': True,
                ...
                print(f"[Query] ✗ Server nicht erreichbar (Port geschlossen)")
        except Exception as e:
            print(f"[Query] Socket Test Fehler: {e}")
        
        # Methode 2: Versuche HTTP API (falls verfügbar)
        try:
            # Einige RageMP Server haben eine Web-API
            for api_port in [port, '80', '8080']:
                try:
                    url = f"http://{host}:{api_port}/api/info"
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url, timeout=5) as response:
                            if response.status == 200:
                                try:
                                    data = await response.json()
                                    print(f"[Query] ✓ API Info erfolgreich abgerufen")
                                    return {
                                        'online': True,
            ...
            print(f"[Query] HTTP API Fehler: {e}")
        
        # Methode 3: Nutze rage.mp Server List (falls Server öffentlich gelistet ist)
        try:
            # Hole Server-Info von rage.mp Server-Browser
            url = "https://rage.mp/master/"
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=10, headers={'Accept': 'application/json'}) as response:
                    if response.status == 200 and 'json' in response.content_type:
                        ...
                        # Suche nach unserem Server
                        for srv in servers:
                            if host.lower() in srv.get('ip', '').lower() or \
                               srv.get('ip') == host or \
                               f"{srv.get('ip')}:{srv.get('port')}" == ip_port:
                                print(f"[Query] ✓ Server in Master List gefunden")
                                return {
                                    'online': True,
                                    'server_name': srv.get('name', 'GTA Grand DE 1'),
                                    'players': srv.get('players', 0),
                                    'max_players': srv.get('maxplayers', 100)
                                }
        except Exception as e:
            # Ignoriere Master List Fehler - nicht kritisch
            pass
        
    except Exception as e:
        print(f"[Query] ✗ Fehler beim Abfragen: {e}")
    
    # Fallback: Server als offline markieren
    print(f"[Query] ✗ Server scheint offline zu sein")
    return {
        'online': False,
        'server_name': 'GTA Grand DE 1',
        'players': 0,
